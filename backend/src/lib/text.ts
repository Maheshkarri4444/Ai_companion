...
export function escapePromptData(value: string): string {
  return value.replace(/<\/?(source|sources|request|learner_context|conversation_summary|tool_result|message|question|answer|material|rubric|item|avoid)\b/gi, (m) =>
    m.replace('<', '‹'),
  );
}
